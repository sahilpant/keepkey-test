...
import { marshalTx, unmarshalTx, encodeString } from '@tendermint/amino-js';

import { amino , BncClient, Transaction} from '@binance-chain/javascript-sdk';
...
import { AminoPrefix, StdSignMsg, StdTx } from "@binance-chain/javascript-sdk/lib/types";
import { marshalBinary } from "@binance-chain/javascript-sdk/lib/amino";
...
$binanceTx.on("click", async (e) => {
  ...
  if (supportsBinance(wallet)) {
    ...
    const Txhex = (bytes.toString("hex"));

    console.log(Txhex);

    const result = await bncClient.sendRawTransaction(Txhex,true);

    console.log(result);

    // const tx =  new Transaction(stdSign);

    // const x = tx.serialize();
    ...
    // const preparedtx = await bncClient._prepareTransaction(res,stdSign,"bnb1w0zp0e2unswlvzn8sdp6me9jyfpt09gxuzsgq0");
    
    // console.log(preparedtx);
    
    // const result = await bncClient.sendTransaction(preparedtx,true);
